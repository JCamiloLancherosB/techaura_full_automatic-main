...
const usbProducts: { [key: string]: USBProduct } = {
    '1': {
        ...
        originalPrice: 79900,
        discount: 25,
        description: '8GB - Pack Básico',
        benefits: ['✅ Ideal para empezar', '✅ Canciones Alta Calidad', '✅ Envío Gratis'],
        urgency: '⚡ Pocas unidades'
    },
    '2': {
        capacity: '32GB',
        songs: '5,000',
        ...
        originalPrice: 119900,
        discount: 25,
        description: '32GB - La Favorita',
        benefits: ['🔥 MÁS VENDIDA', '✅ 5,000 canciones', '✅ Garantía Extendida', '✅ Envío Gratis'],
        urgency: '🔥 Quedan 5 unidades',
        popular: true
    },
    '3': {
        capacity: '64GB',
        ...
        originalPrice: 169900,
        discount: 24,
        description: '64GB - Pack Premium',
        benefits: ['⭐ MEJOR VALOR', '✅ 10,000 canciones + Podcasts', '✅ Audio Estudio', '✅ Envío Rápido'],
    ...
    '4': {
        ...
        originalPrice: 229900,
        discount: 26,
        description: '128GB - Colección VIP',
        benefits: ['👑 SOLO PARA FANÁTICOS', '✅ 22,000 canciones + Videos', '✅ Soporte VIP', '✅ Envío Prioritario'],
...
const additionalProducts: AdditionalProduct[] = [
    {
        ...
        originalPrice: 129900,
        img: 'https://i.imgur.com/audifonos-premium.jpg',
        benefits: ['🎵 Sonido HD', '🔋 20h batería', '🎧 Cancelación ruido'],
        combo: true
    },
    {
        name: 'Cargador Inalámbrico Rápido',
        ...
        originalPrice: 79900,
        img: 'https://i.imgur.com/cargador-wireless.jpg',
        benefits: ['⚡ Carga ultra rápida', '📱 Universal', '🛡️ Seguro'],
        combo: true
    },
    {
        ...
        originalPrice: 59900,
        img: 'https://i.imgur.com/soporte-magnetico.jpg',
        benefits: ['🧲 Súper magnético', '🚗 Para el auto', '📱 Rotación 360']
    },
    {
        name: 'Power Bank 20,000mAh',
        price: 79900,
        originalPrice: 119900,
        img: 'https://i.imgur.com/powerbank-premium.jpg',
        benefits: ['🔋 Carga 5 equipos', '⚡ Carga rápida', '💎 Diseño lujo'],
        combo: true
    },
    {
        ...
        originalPrice: 29900,
        img: 'https://i.imgur.com/cable-premium.jpg',
        benefits: ['💪 Indestructible', '⚡ Carga rápida', '📏 2 metros']
    }
];
...
// ✅ CROSS-SELL MEJORADO (Con Delays)
async function crossSellSuggestion(currentProduct: 'music' | 'video', flowDynamic: any, phoneNumber: string) {
    try {
        const session = await getUserSession(phoneNumber);
        const offerText = currentProduct === 'music'
            ? '🎬 *¿Te gustaría añadir VIDEOS MUSICALES a tu pedido?*'
            : '🎵 *¿Te gustaría añadir MÚSICA a tu pedido?*';

        const detailText = currentProduct === 'music'
            ? '👉 Más de 10,000 videoclips en HD. El complemento perfecto para tu pantalla.'
            : '👉 La mejor selección de playlists organizadas por carpetas.';

        await flowDynamic([
            { body: '⏳ Un momento...', delay: 500 },
            { body: offerText, delay: 1000 },
            { body: detailText, delay: 1000 },
            { body: '🎁 *Oferta Combo:* Si llevas ambas, tienes envío prioritario.\n\nResponde *SÍ* para agregar o *NO* para finalizar.', delay: 1500 }
        ]);

        if (session) {
            await updateUserSession(phoneNumber, 'Cross-sell presentado', 'cross_sell_presented', null, false, { metadata: session });
        }
    } catch (error) {
        console.error('❌ Error en crossSellSuggestion:', error);
    }
...
const capacityComparison = addKeyword(['comparar', 'diferencias', 'cual elegir', 'opciones'])
    .addAction(async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        try {
            const phoneNumber = ctx.from;
            const pre = await preHandler(ctx, { flowDynamic, gotoFlow }, 'musicUsb', ['prices_shown', 'awaiting_capacity', 'personalization'], { lockOnStages: ['awaiting_payment', 'checkout_started', 'completed'] });
            if (!pre.proceed) return;

            // OPTIMIZACIÓN: Mensaje partido para facilitar lectura
            await flowDynamic([
                { body: '📊 *Comparativa Rápida:*', delay: 500 },
                { body: '🎵 *32GB ($89.900)*: La opción estándar. Perfecta si escuchas música variada en el carro. (5,000 canciones).', delay: 1000 },
                { body: '⭐ *64GB ($129.900)*: La opción inteligente. El doble de espacio por solo un poco más. (10,000 canciones).', delay: 1500 },
                { body: '👑 *128GB ($169.900)*: La opción definitiva. Música ilimitada + Videos. Para coleccionistas. (22,000 archivos).', delay: 2000 },
                { body: '¿Cuál se ajusta mejor a ti? Responde *2, 3 o 4*.', delay: 1500 }
            ]);
    ...
    .addAction({ capture: true }, async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        ...
        if (isValidSelection(selection)) {
            ...
            await flowDynamic([{ body: '❌ Por favor escribe solo el número: 2, 3 o 4.', delay: 1000 }]);
        }
    });

// --- FLUJO PRINCIPAL: SELECCIÓN DE CAPACIDAD ---
const capacityMusic = addKeyword([EVENTS.ACTION])
    // PARTE 1: MOSTRAR OPCIONES (Visual y Persuasivo)
    .addAction(async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        capacityMiddleware
        try {
            const phoneNumber = ctx.from;
            const pre = await preHandler(ctx, { flowDynamic, gotoFlow }, 'musicUsb', ['awaiting_capacity', 'prices_shown', 'personalization'], { lockOnStages: ['awaiting_payment', 'checkout_started', 'completed'] });
            if (!pre.proceed) return;

            const session = await getUserSession(phoneNumber);
            if (processingUsers.has(phoneNumber)) return;

            // Lógica anti-spam de re-renderizado
            const now = Date.now();
            if (session?.lastProcessedTime && session.currentFlow === 'capacity_music' && (now - new Date(session.lastProcessedTime).getTime()) < 5000) return;

            if (session) {
                session.currentFlow = 'capacity_music';
                session.lastProcessedTime = new Date();
                await updateUserSession(phoneNumber, 'Iniciando selección', 'capacity_flow_start', null, false, { metadata: session });
            }

            // OPTIMIZACIÓN: Mensajes estructurados con delay para efecto "Chat Real"
            await flowDynamic([
                { body: '🎶 *¡Tu USB personalizada está lista para armarse!*', delay: 500 },
                { body: 'Hemos aplicado el *Descuento Especial de Hoy*. Elige tu capacidad ideal:', delay: 1000 },

                // Opciones Básicas
                { body: '🔹 *Opción 1: 8GB* (1,400 canciones)\n💰 *$59.900* (Antes $79.900)\n_Perfecta para empezar._', delay: 1500 },

                // Opción Estrella
                { body: '🔥 *Opción 2: 32GB* (5,000 canciones)\n💰 *$89.900* (Antes $119.900)\n⭐ _La más vendida para carros._', delay: 1500 },

                // Opciones Premium
                { body: '💎 *Opción 3: 64GB* (10,000 canciones)\n💰 *$129.900* (Antes $169.900)\n🚀 _Máxima velocidad y capacidad._', delay: 1500 },
                { body: '👑 *Opción 4: 128GB* (22,000 canciones + Videos)\n💰 *$169.900* (Antes $229.900)\n🎬 _La colección definitiva._', delay: 1500 },

                // CTA
                { body: '👇 Escribe el número de tu elección (*1, 2, 3 o 4*) para continuar.', delay: 2000 }
            ]);

            await postHandler(phoneNumber, 'musicUsb', 'awaiting_capacity');

        } catch (error) {
            console.error('❌ Error en capacityMusic:', error);
            await flowDynamic(['⚠️ Error técnico. Escribe el número de opción (1-4).']);
        }
    })
    // PARTE 2: CAPTURAR Y CONFIRMAR
    .addAction({ capture: true }, async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        try {
            const phoneNumber = ctx.from;
            const pre = await preHandler(ctx, { flowDynamic, gotoFlow }, 'musicUsb', ['awaiting_capacity', 'awaiting_payment'], { lockOnStages: ['checkout_started', 'completed'] });
            if (!pre.proceed) return;

            const input = ctx.body?.trim()?.toLowerCase() || '';

            // Mapeo de input a producto
            let productKey = '';
            if (input.includes('1') || input.includes('8')) productKey = '1';
            else if (input.includes('2') || input.includes('32')) productKey = '2';
            else if (input.includes('3') || input.includes('64')) productKey = '3';
            else if (input.includes('4') || input.includes('128')) productKey = '4';
            else if (input.includes('comparar')) return gotoFlow(capacityComparison);
            else {
                await flowDynamic([{ body: '❌ No entendí esa opción. Por favor escribe solo el número *1, 2, 3 o 4*.', delay: 1000 }]);
                await postHandler(phoneNumber, 'musicUsb', 'awaiting_capacity');
                return;
            }
            ...
            const product = usbProducts[productKey];
            const session = await getUserSession(phoneNumber);
            const genero = session?.conversationData?.selectedGenre || 'Variado';
            const savings = calculateSavings(product.originalPrice, product.price);

            // Guardar sesión y contexto
            await updateUserSession(phoneNumber, `Capacidad: ${product.capacity}`, 'capacityMusic', null, false, {
                metadata: { step: 'order_summary', productType: 'music', selectedCapacity: product.capacity, price: formatPrice(product.price), orderReady: true }
            });
            ...
            localUserSelections[phoneNumber] = {
                capacity: product.capacity,
                description: `${product.capacity} (${product.songs} canciones)`,
                price: product.price,
                originalPrice: product.originalPrice,
                savings: savings,
                timestamp: new Date(),
            ...
            // OPTIMIZACIÓN: Confirmación emocionante + Cross-sell inmediato
            await flowDynamic([
                { body: `🎉 *¡Excelente elección!* La de ${product.capacity} es genial.`, delay: 500 },
                { body: `✅ Estás ahorrando *${savings}* en tu compra hoy.`, delay: 1000 },
                { body: `🎁 Incluye: ${product.benefits.join(' + ')}.`, delay: 1000 }
            ]);

            // Lanzar Cross-sell (Videos o Música complementaria)
            await crossSellSuggestion('music', flowDynamic, phoneNumber);
            ...
            console.error('❌ Error captura capacity:', error);
            await flowDynamic(['⚠️ Algo salió mal. Intenta de nuevo.']);
        }
    });
...
const askShippingData = addKeyword([EVENTS.ACTION])
    .addAction(async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        try {
            const phoneNumber = ctx.from;
            const pre = await preHandler(ctx, { flowDynamic, gotoFlow: async () => { } }, 'musicUsb', ['awaiting_payment', 'checkout_started'], { lockOnStages: ['completed'] });
            if (!pre.proceed) return;

            await updateUserSession(phoneNumber, 'Solicitando datos', 'shipping_data_request', null, false);

            await flowDynamic([
                { body: '🔒 *Aseguremos tu envío (Es Gratis)*', delay: 500 },
                { body: 'Por favor escribe tus datos de entrega en un solo mensaje:', delay: 1000 },
                { body: '📝 *Nombre, Ciudad, Dirección y Celular*\n\n_Ejemplo: Juan Pérez, Bogotá, Calle 123 #45-6, 3001234567_', delay: 1500 }
            ]);

            await postHandler(phoneNumber, 'musicUsb', 'awaiting_payment');
        } catch (error) {
            console.error('❌ Error askShippingData:', error);
        }
    })
    .addAction({ capture: true }, async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        try {
            ...
            if (shippingData.length < 15) {
                await flowDynamic([{ body: '❌ Los datos parecen muy cortos. Por favor incluye Nombre, Ciudad y Dirección.', delay: 1000 }]);
                return;
            }
            ...
            // Convertir a carrito y marcar completado
            if (session) {
                const selection = localUserSelections[phoneNumber];
                if (selection) session.cartData = convertSelectionToCart(phoneNumber, selection);
                await updateUserSession(phoneNumber, 'Datos recibidos', 'completed', null, false, { metadata: session });
            }

            await flowDynamic([
                { body: '✅ *¡Datos recibidos!*', delay: 500 },
                { body: 'Tu pedido está prácticamente listo. Un asesor lo revisará en breve.', delay: 1000 }
            ]);

            await postHandler(phoneNumber, 'musicUsb', 'checkout_started');
            return gotoFlow(showAdditionalProducts); // Intentar Upsell final

        } catch (error) {
            console.error('❌ Error shipping capture:', error);
            await flowDynamic(['⚠️ Error guardando datos. Intenta nuevamente.']);
        }
    });

// --- FLUJO UPSELL FINAL (Productos Adicionales) ---
const showAdditionalProducts = addKeyword([EVENTS.ACTION])
    .addAction(async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        try {
            const phoneNumber = ctx.from;
            const pre = await preHandler(ctx, { flowDynamic, gotoFlow }, 'musicUsb', ['checkout_started', 'awaiting_payment'], { lockOnStages: ['completed'] });
            if (!pre.proceed) return;

            if (!localUserSelections[phoneNumber]) return gotoFlow(orderProcessing);

            const comboProducts = getRandomProducts(2, true);

            // Mostrar productos con imagen si es posible
            await flowDynamic([{ body: '🛍️ *Oferta Flash VIP:* Antes de cerrar, mira esto:', delay: 1000 }]);

            for (const [index, product] of comboProducts.entries()) {
                const msg = {
                    body: `${index + 1}️⃣ *${product.name}*\n💰 Oferta: ${formatPrice(product.price)} (Normal: ${formatPrice(product.originalPrice)})\n✨ ${product.benefits[0]}`,
                    media: product.img, // Si el bot soporta media url
                    delay: 1000
                };
                await flowDynamic([msg]);
            }

            await flowDynamic([{ body: '¿Quieres agregar alguno al envío (Sin costo extra)?\nResponde *1*, *2* o *NO* para finalizar.', delay: 2000 }]);

            await updateUserSession(phoneNumber, 'Upsell mostrado', 'additional_products_shown', null, false);
            await postHandler(phoneNumber, 'musicUsb', 'checkout_started');

        } catch (error) {
            console.error('❌ Error upsell:', error);
            return gotoFlow(orderProcessing);
        }
    })
    .addAction({ capture: true }, async (ctx: BotContext, { flowDynamic, gotoFlow }: any) => {
        ...
        if (['no', 'n', 'gracias', 'omitir'].some(r => response.includes(r))) {
            await flowDynamic([{ body: '👍 ¡Perfecto! Procesando tu pedido principal...', delay: 1000 }]);
        } else if (['1', '2'].includes(response)) {
            const product = getRandomProducts(2, true)[parseInt(response) - 1]; // Nota: Esto debe sincronizarse mejor en prod, pero sirve para demo
            if (product && localUserSelections[phoneNumber]) {
                localUserSelections[phoneNumber].additionalProducts = localUserSelections[phoneNumber].additionalProducts || [];
                localUserSelections[phoneNumber].additionalProducts!.push(product.name);

                // Actualizar carrito real
                if (session) session.cartData = convertSelectionToCart(phoneNumber, localUserSelections[phoneNumber]);

                await flowDynamic([{ body: `✅ *${product.name}* agregado con éxito.`, delay: 1000 }]);
            }
        }
...
// --- UTILIDADES DE EXPORTACIÓN (Sin cambios funcionales, solo estructura) ---
export const getUserSelectionData = (phoneNumber: string) => localUserSelections[phoneNumber];
export const getUserCartData = (phoneNumber: string) => {
    try {
        const selection = localUserSelections[phoneNumber];
        return selection ? convertSelectionToCart(phoneNumber, selection) : null;
    } catch { return null; }
};
export const clearUserSelection = (phoneNumber: string) => {
    if (localUserSelections[phoneNumber]) {
        delete localUserSelections[phoneNumber];
        processingUsers.delete(phoneNumber);
        return true;
...
export const getOrderSummary = (phoneNumber: string) => {
    const selection = localUserSelections[phoneNumber];
    if (!selection) return null;
    let summary = `📋 *Resumen:*\n${selection.description}\n💰 ${formatPrice(selection.price)}`;
    if (selection.additionalProducts?.length) {
        summary += `\n🛍️ +${selection.additionalProducts.length} Extras`;
    }
    return summary;
};
export const getSystemMetrics = () => ({
    active: Object.keys(localUserSelections).length,
    processing: processingUsers.size
});

export default capacityMusic;
export { capacityComparison, askShippingData, showAdditionalProducts, usbProducts, additionalProducts, formatPrice, calculateSavings, convertSelectionToCart };
