    ...
    genreTopHits:{
        ...
        "rock": [
            ...
            { name: "Queen - Bohemian Rhapsody", file: '../demos/Rock/recortado_System Of A Down - Toxicity (Official HD Video).mp3' }
                ...
                // Puedes agregar otros géneros vacíos o con canciones según sea necesario
            },
        playlistImages:{
    ...
    class IntentDetector {
        ...
        static extractArtists(message: string, genres: string[] = []): string[] {
            ...
            genresToSearch.forEach(genre => {
                musicData.artistsByGenre[genre]?.forEach(artist => {
    ...
    async function persistOrderProgress(phoneNumber: string, data: Partial<UserCustomizationState>) {
        ...
        if (!userSessions[phoneNumber]) userSessions[phoneNumber] = {};
        Object.assign(userSessions[phoneNumber], {
    ...
    async function handleObjections(userInput: string, flowDynamic: any) {
        ...
        if (/precio|caro|costoso|vale|es mucho/i.test(input)) {
            await flowDynamic([
                '💡 *¡Recuerda! El precio incluye TODO: 5,000 canciones, playlist personalizada, envío gratis y garantía de 30 días.*\n\n🎉 *Y hoy puedes pagar en cuotas sin intereses!*'
            ]);
            return true;
        }
        if (/demora|tarda|cuanto demora|cuánto demora|cuánto tiempo/i.test(input)) {
            await flowDynamic([
                '⏱️ *¡Enviamos el mismo día!* Tu USB llega en 1-3 días hábiles a cualquier ciudad de Colombia.'
            ]);
            return true;
        }
        if (/confio|seguro|garantía|fraude|es real/i.test(input)) {
            await flowDynamic([
                '✅ *Compra 100% segura:* Somos tienda oficial y tienes garantía de satisfacción 30 días o te devolvemos el dinero.'
            ]);
            return true;
        }
        return false;
    ...
    async function suggestUpsell(phoneNumber: string, flowDynamic: any, userState: UserCustomizationState) {
        if (!userState.upsellOfferSent) {
            ...
            await flowDynamic([
                '🎬 *¡Oferta especial!* Añade la USB de VIDEOS MUSICALES con +10,000 videoclips HD por solo $39.900 más. ¿Te gustaría agregarla y tener el pack completo? ✅ Escribe "QUIERO PACK COMPLETO" o "SOLO AUDIO".'
            ]);
        }
    }
    ...
    async function offerQuickPayment(phoneNumber: string, flowDynamic: any, userState: UserCustomizationState) {
        ...
        await UserStateManager.save(userState);
        await flowDynamic([
            '🛒 *¡Último paso!*\n\n✅ ¿Listo para tu USB personalizada? Puedes pagar fácil con Nequi, Bancolombia, Daviplata, tarjeta o contra entrega. ¿Confírmame tu ciudad de envío o escribe "PAGAR" para recibir el enlace de pago inmediato.'
        ]);
    }
    ...
    const musicUsb = addKeyword([
        ...
        .addAction(async (ctx, { flowDynamic }) => {
            ...
            try {
                ...
                // Actualizar el flujo actual y el estado de la sesión
                session.currentFlow = 'musicUsb'; // Mantener el flujo actual
                session.isActive = true;

                // 1. Engagement Inmediato
                await flowDynamic([
                    `🚀 *¡Bienvenido! [Estamos de oferta]*\n\n🎶 *¡USB musical personalizada, envío gratis y pagas solo cuando recibes!*\n\n🔥 *Quedan pocas unidades HOY*\n\n${Math.random() > 0.5 ? '🎉 ¡Tu música lista en minutos!' : '⭐ Más de 1,800 clientes felices hoy'}`
                ]);
                await MusicUtils.delay(700);

                // 2. Presentación rápida del producto estrella
                ...
                // 4. Pregunta de diagnóstico inmediata para segmentar y personalizar
                await flowDynamic([
                    // '🎵 *Para personalizar tu USB, dime tus gustos:*',
                    // '- ¿Qué géneros musicales te gustan más? Por ejemplo: "rock y salsa", "reggaeton y bachata", "toda la música romántica".',
                    // '- ¿Tienes artistas favoritos? Ejemplo: "Karol G y Bad Bunny", "Marc Anthony y Metallica".',
                    '',
                    '✅ *Escribe tus géneros/artistas favoritos, o simplemente "OK" para recibir una música crossover y el precio especial de hoy.*',
                    // '💰 *Si deseas saber el precio, escríbelo y te lo digo directo sin rodeos!*'
                ]);
                // Tracking y estado
        ...
        .addAction({ capture: true }, async (ctx, { flowDynamic, gotoFlow }) => {
            ...
            await flowDynamic([
                '🎵 ¡Perfecto! Armando tu música crossover...',
                // '🔥 OFERTA ESPECIAL: USB 32GB con 5,000 canciones por solo $89.900 hoy',
                // '¿Qué capacidad prefieres? (32GB, 64GB o 128GB)'
            ]);
            session.currentFlow = 'recommendedPlaylist';
            return gotoFlow(capacityMusic); // Redirige al flujo de capacidad
        }

            if (/precio|cuánto|vale|cost[oá]/i.test(userInput)) {
                await flowDynamic([
                    '💰 *El precio especial hoy es:*',
                    ...
                    '✅ ¿Qué géneros o artistas quieres? O dime "OK" para la música crossover.'
                ].join('\n'));
                ProcessingController.clearProcessing(phoneNumber);
                return;
            }
            ...
            try {
                ...
                // -- Upselling directo si el usuario muestra alta intención --
                if (/pack completo|quiero ambos|quiero video/i.test(userInput)) {
                    await flowDynamic([
                        '🎁 *¡Perfecto! Te aparto la USB de música + la de VIDEOS HD con 25% off. ¡Preparamos tu combo VIP!*'
                    ]);
                    ProcessingController.clearProcessing(phoneNumber);
                ...
                if (['1', '2', '3', '4'].includes(userInput.trim()) ||
                    /para mi|para mí|para mamá|para papa|para papá|entrenar|trabajar|personal|regalo|viaje|fiesta|uso/i.test(MusicUtils.normalizeText(userInput))) {
                    let perfil = '';
                    ...
                    await UserStateManager.save(userState);
                    await flowDynamic([
                        `🙌 *¡Genial! Personalizaremos tu USB para ${perfil}.*\n\n*¿Qué géneros o artistas te gustan más?*\nEjemplo: "rock y salsa", "Karol G y Bad Bunny", o escribe OK para la música crossover.`
                    ]);
                    ProcessingController.clearProcessing(phoneNumber);
                    return;
                }

                // --- AVANZADA: Géneros/artistas/mood ---
                ...
                if (userGenres.length > 0 || userArtists.length > 0 || moodKeywords.length > 0) {
                    ...
                    let resp = `🎵 *¡Excelente selección!*\nIncluiremos tus favoritos:\nGéneros: ${userState.selectedGenres.join(', ') || '-'}\nArtistas: ${userState.mentionedArtists.join(', ') || '-'}\n\n¿Te gustaría agregar más géneros o artistas, o seguimos?\n✅ *Escribe OK para continuar* o dime más géneros/artistas.`;
                ...
                // --- AVANCE CON OK (cuando sí hay preferencias ya guardadas) ---
                if (IntentDetector.isContinueKeyword(userInput)) {
                    const userState = await UserStateManager.getOrCreate(ctx.from);
            
                    if (userState.selectedGenres.length === 0 && userState.mentionedArtists.length === 0) {
                        await flowDynamic([
                            '🎵 ¡Perfecto! Armando tu música crossover...',
                            // '🔥 OFERTA ESPECIAL: USB 32GB con 5,000 canciones por solo $89.900 hoy',
                            // '¿Qué capacidad prefieres? (32GB, 64GB o 128GB)'
                    ...
                    if ((userState.unrecognizedResponses || 0) >= 1) {
                        userState.finalizedGenres = musicData.playlistsData[0].genres;
                        userState.finalizedArtists = [];
                        userState.finalizedOrderAt = new Date().toISOString();
                        userState.unrecognizedResponses = 0;
                        await UserStateManager.save(userState);
                        await persistOrderProgress(phoneNumber, {
                            finalizedGenres: userState.finalizedGenres,
                            finalizedArtists: userState.finalizedArtists,
                            finalizedOrderAt: userState.finalizedOrderAt,
                        });
                        await flowDynamic([
                            '✅ ¡Listo! Te armo la música crossover y el precio especial.',
                            'Elige la capacidad ideal para ti:',
                    ...
                    await UserStateManager.save(userState);
                    await flowDynamic([
                        '🙋‍♂️ Para personalizar tu playlist y aprovechar el descuento, dime al menos 1 género o artista que te guste (ejemplo: "reggaeton", "Karol G", "rock").\nSi realmente quieres la música crossover, responde "OK" otra vez.'
                    ]);
                    ProcessingController.clearProcessing(phoneNumber);
                    return;
                }

                session.currentFlow = session.currentFlow; // Mantener el flujo actual
                session.isActive = true;

                // --- Cierre inmediato si intención de compra alta ---
                ...
                // --- Mensaje de urgencia si no se reconoce nada de lo anterior ---
                userState.unrecognizedResponses = (userState.unrecognizedResponses || 0) + 1;
                userState.touchpoints = [...(userState.touchpoints || []), 'unrecognized_response'];
                await UserStateManager.save(userState);
                if (userState.unrecognizedResponses >= 1) {
                    await flowDynamic([
                        '🙋‍♂️ Para personalizar tu playlist y aprovechar el descuento, dime al menos 1 género o artista que te guste (ejemplo: "reggaeton", "Karol G", "rock").\nSi realmente quieres la música crossover, responde "OK" otra vez.'
                    ]);
                    ProcessingController.clearProcessing(phoneNumber);
